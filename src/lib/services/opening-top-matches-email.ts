    ...
    <div dir="rtl" style="font-family:Arial,sans-serif;background:#f5f8fb;padding:32px;">
      <div style="max-width:760px;margin:0 auto;background:white;border-radius:28px;padding:32px;border:1px solid #d7e4f0;box-shadow:0 20px 60px rgba(15,23,42,0.08);">
        ...
        <p style="margin:16px 0 0;font-size:16px;line-height:1.8;color:#334155;">
          ${input.ownerName ? `${input.ownerName}, ` : ""}הדדליין למשרה <strong>${input.opening.title}</strong> הגיע.
          ...
          input.selectedApplicants.length > 0
            ...
            : `
              <div style="margin-top:28px;padding:18px 20px;border-radius:20px;background:#fff7ed;color:#7c2d12;line-height:1.8;">
                לא נקלטו מועמדויות מתאימות למשלוח בתקציר הזה. אפשר להיכנס למשרה ולבדוק אם נוספו חומרים ידנית.
              </div>
            `
        }

        <a href="${reviewUrl}" style="display:inline-block;margin-top:28px;padding:14px 20px;border-radius:999px;background:#0b5fb5;color:white;font-weight:700;text-decoration:none;">לניהול המשרה המלאה</a>
      </div>
    </div>
  `;

  const text = [
    `${input.ownerName ? `${input.ownerName}, ` : ""}הדדליין למשרה ${input.opening.title} הגיע.`,
