...
export function buildDepartmentCopy(input: {
  ...
  if (input.institutionType === InstitutionType.HMO) {
    return {
      ...
      about: `${baseName} ב${input.institutionName}, עם מידע שמיועד לעזור להבין את סביבת העבודה, המסלול וההקשר הקהילתי.`,
      practicalInfo: `במסלולי קהילה מוצגים בעיקר תחומי רפואת משפחה ורפואת קהילה, לצד מידע שימושי שמסייע להשוואה בין מסגרות.`
    };
  }

  return {
    shortSummary: `${baseName} ב${input.institutionName} עם מידע להשוואה בין מחלקות, חוויות ותוכן רשמי כאשר הוא זמין.`,
    about: `${baseName} ב${input.institutionName}. העמוד מרכז מידע שימושי להשוואה, קריאת חוויות מהשטח ושמירת המחלקה להמשך בדיקה.`,
    practicalInfo: `המידע בעמוד נועד לתת נקודת פתיחה מסודרת לבחינת המחלקה, לצד מקורות רשמיים ושיתופים שעוברים בדיקה.`
  };
}
  ...
  return db.institution.create({
    data: {
      ...
      summary:
        ...
        "מוסד רפואי המופיע במאגר ההתמחויות לצורך שיוך מחלקות והשוואה בין אפשרויות."
    }
  });
}
  ...
  return db.specialty.create({
    data: {
      ...
      description:
        ...
        "תחום התמחות המופיע במאגר לצורך השוואה בין מחלקות, נתונים ושיתופים מהשטח."
    }
  });
}
