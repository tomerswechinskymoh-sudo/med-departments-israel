...
addCheck("calendar selected range uses explicit priority branches", electivesCatalog.includes("buttonVisualClass = isSelectedEdge") && electivesCatalog.includes(": inFinalSelectedRange") && electivesCatalog.includes(": inPreviewRange"));
addCheck("calendar selected range uses strong readable styles", electivesCatalog.includes("bg-brand-700") && electivesCatalog.includes("bg-brand-900") && electivesCatalog.includes("disabled:text-white") && electivesCatalog.includes("disabled:opacity-100"));
addCheck("calendar selected day numbers force visible white text", electivesCatalog.includes("dayNumberClass = isSelectedEdge") && electivesCatalog.includes("text-white font-black opacity-100") && electivesCatalog.includes("text-white font-semibold opacity-100"));
addCheck("selected range avoids low-opacity text", electivesCatalog.includes("isOutsideMonth && !inSelectedRange") && electivesCatalog.includes("relative z-10 block text-sm") && !electivesCatalog.includes("inFinalSelectedRange && !isSelectedEdge"));
addCheck("calendar exposes selected range debug attributes", electivesCatalog.includes("data-day-label={day}") && electivesCatalog.includes("data-selected-start") && electivesCatalog.includes("data-selected-end") && electivesCatalog.includes("data-selected-range"));
