...
export default async function ClinicalRotationsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <PageShell className="space-y-6 py-8">
          ...
          <Card key={offering.id} className="flex flex-col justify-between gap-4">
            <div className="space-y-3">
              ...
              <p className="text-xs font-bold text-slate-500">
                משתתפים מאושרים: {offering.participantCount} / מינימום {offering.minimumParticipants}
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
      ...
      {offerings.length === 0 ? (
        <Card>
          <p className="text-sm font-semibold text-slate-700">אין כרגע סבבים פתוחים שתואמים לסינון.</p>
